...
class Upload
{
     ...
     */
    public function __construct($path = '', $ext = array(), $size = array(), $waterMarkOn = false, $thumbOn = false, $thumb = array())
    {
        ...
        $this->waterMarkOn = empty($waterMarkOn) ? C("WATER_ON") : $waterMarkOn;
        $this->thumbOn = empty($thumbOn) ? C("UPLOAD_THUMB_ON") : $thumbOn;
        $this->thumb = $thumb;
    }
    ...
    private function save($file)
    {
        ...
        $arr = array();
        if ($this->thumbOn) {
            $thumbFile = ltrim(str_replace(ROOT_PATH, '', $thumbFile), '/');
