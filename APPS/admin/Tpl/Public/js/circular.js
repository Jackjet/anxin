 ...
 function ajax_page(page){
        ...
        url=app+"/Circular/ajaxShowPage/do/list/";
        ...
        $.post(url,{page:page},function(data){
            ...
            html +='<td class="td_title"><a href="'+public+'/circulars/zh/'+list[i].file_url+'" target="_blank">'+list[i].title+'</a></td>';

			html +='<td class="td_title"><a href="'+public+'/circulars/en/'+list[i].file_url_en+'" target="_blank">'+list[i].title_en+'</a></td>';

            html +='<td class="del"><span><a href="javascript:ajax_del_circular('+list[i].id+')">删除</a></span></td>';

            html +='<td class="edit"><span><a href="'+app+'/Circular/toUpdate?id='+list[i].id+'">修改</a></span></td>';
  ...
  function ajax_del_circular(id){
  		//alert(id);
      url=app+"/Circular/ajaxDelCircular/do/list/";
	  ...
	  if(isDelFile){
		  ...
		  $.post(url,{"page":$page,"id":id},function(data){
			  ...
			  if(status==0){
				...
				html +='<td class="td_title"><a href="'+public+'/circulars/en/'+list[i].file_url+'" target="_blank">'+list[i].title_en+'</a></td>';
