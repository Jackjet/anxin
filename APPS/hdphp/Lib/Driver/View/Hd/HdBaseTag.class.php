...
//    static $tagArr = array("list" => array()); //标签数组集
...
//        $str .= "SESSION_NAME='<?php echo session_name();? >';\n";
//        $str .= "SESSION_ID='<?php echo session_id();? >';\n";
        $str .= "</script>";
        return $str;
    }
    ...
    public function _upload($attr, $content)
    {
        ...
        if ($data) {
            ...
            $imgData = HdView::$vars[$varName];
            foreach ($imgData as $k => $_img) {
                if (empty($_img['path'])) {
                    //删除path为空的图片元素
                    unset(HdView::$vars[$varName][$k]);
                }
            }
        }
        ...
        if ($data) {
            //编辑时统计图片数量
            $uploadsSuccessful = count(HdView::$vars[$varName]);
        } else {
            //上传时初始上传成功文件为0
            $uploadsSuccessful = 0;
        }
        ...
        if ($data) {
            ...
            $upFileId=0;//第几张图片
            //读取图片数据
            foreach (HdView::$vars["' . $varName . '"] as $f) {
                $upFileId++;
                $url = \'__ROOT__/\' . $f["path"];
...
<input type=\'text\'  value=\'".$f["alt"]."\' name=\'' . $name . '[".$upFileId."][alt]\'>
        ...
        if (!$_hd_uploadify_js) {
            ...
            $uploadTotal = 0;
            if ($data) {
                $uploadTotal = count(HdView::$vars[$varName]);
            }
...
<div class="' . $id . '_files uploadify_upload_files" input_file_id ="' . $id . '">
    ...
    public function _js($attr, $content)
    {
        ...
        switch ($file) {
            ...
            case 'dialog':
                ...
                $str .= "<script type='text/javascript' src='" . __HDPHP_EXTEND__ . "/Html/JavaScript/Hd/dialog.js'></script>\n";
                $str .= "<link  rel='stylesheet' type='text/css' href='" . __HDPHP_EXTEND__ . "/Extend/Hd/css/dialog.css'/>";
                return $str;
        }
    ...
    public function _load($attr, $content)
    {
        ...
        $attr = tpl_format_attr($attr);
        $file = str_replace(__ROOT__, ROOT_PATH, trim($attr['file']));
        $view = new HdView();
        $view->fetch($file);
