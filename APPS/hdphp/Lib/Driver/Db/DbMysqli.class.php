...
class DbMysqli extends Db
{
    ...
    function getLink()
    {
        if (is_null(self::$dbLink)) {
            ...
            if (mysqli_connect_errno()) {
                throw_exception(mysqli_connect_error() . L("mysqlidriver_connectDb"), false); //数据库连接出错了请检查配置文件中的参数
            }
            self::setCharts();
        }
    ...
    {
        if ($this->link->error) {
            throw_exception($this->link->error.'<br/><br/>'.array_pop(Debug::$sqlExeArr));
        }
    }
    ...
    {
        if ($this->link->errno) {
            throw_exception(intval($this->link->errno));
        }
    }
