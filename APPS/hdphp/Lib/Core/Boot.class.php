...
final class Boot
{
    ...
    static public function mkDirs()
    {
        ...
        foreach ($dirs as $d) {
            if (is_dir($d) || dir_create($d, 0777, true)):
            else:
    ...
    {
        //自动加载文件列表
        $files = array_filter(preg_split('@,@i', C("AUTO_LOAD_FILE")));
        $compile = '';
        if (!empty($files)) {
            foreach ($files as $f) {
        ...
        //将公共文件编译
        $compile = self::$_compile . $compile;
        //编译内容
        $compile = '<?php ' . $compile . 'HDPHP::init();App::run();?>';
        //创建编译文件
        if (is_dir(TEMP_PATH) or dir_create(TEMP_PATH,0777, true) and is_writable(TEMP_PATH))
