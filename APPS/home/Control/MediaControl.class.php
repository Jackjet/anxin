...
/************************************************************************************/
   ...
   * @return [type] [description]
   */
  private function handle($type,$table='press_2'){
    $db = M($table);
    include_once(APP_PATH.'/Extend/Lib/myPage.class.php');
    $where = "isshow=1 and type=$type";
    $num = $db->where($where)->count();
    $page = new page($num,PAGE_ROWS);
  
    $result = $db->field("id,title,title_en,file_url,file_url_en,create_time,url")->where($where)->order("orders desc,create_time desc,id desc")->limit($page->firstRow,$page->listRows)->all();
    ...
    $this->assign("page",$page->show());//英文分页
...
/************************************************************************************/
  ...
  function ajaxPage(){
        ...
        if($_GET['ajax'] == 1){
          ...
          if ($data['data'] = $db->field("id,title,title_en,file_url,file_url_en,create_time,url")->where($where)->order("orders desc,create_time desc,id desc")->limit($page->firstRow.','.$page->listRows)->all()) {
