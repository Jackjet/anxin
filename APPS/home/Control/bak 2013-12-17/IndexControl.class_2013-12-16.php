...
class IndexControl extends CommonControl{
    function index(){
        ...
        // C("DEBUG",true);//为TRUE时显示DEBUG信息否则显示按钮
                             
       // C("DEBUG_SHOW",0);//为TRUE时显示DEBUG信息否则显示按钮
    ...
    public function saveStockCode(){
        if(IS_AJAX){
          $data = @$_POST['data'];
          if(!empty($data)){
            $s = file_put_contents(ROOT_PATH.'Public/hk01149.txt', $data);
    ...
    public function getStaticCode(){
        if(IS_AJAX){
            if(is_file(ROOT_PATH.'Public/hk01149.txt')){
              $data = file_get_contents(ROOT_PATH.'Public/hk01149.txt');
              $this->_ajax($data);
            }
